...
instantiation Quantity_ext :: (times, usys, times) times
begin
...
instantiation Quantity_ext :: (zero, usys, zero) zero
begin
...
instantiation Quantity_ext :: (one, usys, one) one
begin
...
instantiation Quantity_ext :: (inverse, usys, inverse) inverse
begin
...
instance Quantity_ext :: (comm_monoid_mult, usys, comm_monoid_mult) comm_monoid_mult
  by (intro_classes, simp_all add: eq_unit one_Quantity_ext_def times_Quantity_ext_def mult.assoc
     ,simp add: mult.commute)

instance Quantity_ext :: (ab_group_mult, usys, ab_group_mult) ab_group_mult
...
instantiation Quantity_ext :: (ord, usys, ord) ord
begin
...
instance Quantity_ext :: (order, usys, order) order
...
instantiation Quantity_ext :: (plus, usys, plus) plus
begin
...
instantiation Quantity_ext :: (uminus, usys, uminus) uminus
begin
...
instantiation Quantity_ext :: (minus, usys, minus) minus
begin
...
definition coerceQuantT :: "'d\<^sub>2 itself \<Rightarrow> 'a['d\<^sub>1::dim_type, 's::usys] \<Rightarrow> 'a['d\<^sub>2::dim_type, 's]" where
...
lift_definition qless_eq :: "'n::order['a::dim_type, 's::usys] \<Rightarrow> 'n['b::dim_type, 's] \<Rightarrow> bool" (infix "\<lesssim>\<^sub>Q" 50) 
  is "(\<le>)" .

lift_definition qequiv :: "'n['a::dim_type, 's::usys] \<Rightarrow> 'n['b::dim_type, 's] \<Rightarrow> bool" (infix "\<cong>\<^sub>Q" 50) 
  is "(=)" .
...
lemma updown_eq_iff:
  fixes x :: "'a['d\<^sub>1::dim_type, 's::usys]" fixes y :: "'a['d\<^sub>2::dim_type, 's]"
...
lift_definition 
  qtimes :: "('n::comm_ring_1)['a::dim_type, 's::usys] \<Rightarrow> 'n['b::dim_type, 's] \<Rightarrow> 'n['a \<cdot>'b, 's]" (infixl "\<^bold>\<cdot>" 69) 
  is "(*)" by (simp add: dim_ty_sem_DimTimes_def times_Quantity_ext_def)

lift_definition 
  qinverse :: "('n::field)['a::dim_type, 's::usys] \<Rightarrow> 'n['a\<^sup>-\<^sup>1, 's]" ("(_\<^sup>-\<^sup>\<one>)" [999] 999) 
  is "inverse" by (simp add: inverse_Quantity_ext_def dim_ty_sem_DimInv_def)

abbreviation (input)
  qdivide :: "('n::field)['a::dim_type, 's::usys] \<Rightarrow> 'n['b::dim_type, 's] \<Rightarrow> 'n['a/'b, 's]" (infixl "\<^bold>'/" 70) where
...
lift_definition scaleQ :: "'a \<Rightarrow> 'a::comm_ring_1['d::dim_type, 's::usys] \<Rightarrow> 'a['d, 's]" (infixr "*\<^sub>Q" 63)
...
instantiation QuantT :: (zero, dim_type, usys) zero
begin
...
instantiation QuantT :: (one, dim_type, usys) one
begin
...
abbreviation qone :: "'n::one[\<one>, 's::usys]" ("\<one>") where "qone \<equiv> 1"
...
instantiation QuantT :: (plus, dim_type, usys) plus
begin
...
instance QuantT :: (semigroup_add,dim_type,usys) semigroup_add
  by (intro_classes, transfer, simp add: add.assoc)

instance QuantT :: (ab_semigroup_add,dim_type,usys) ab_semigroup_add
  by (intro_classes, transfer, simp add: add.commute)

instance QuantT :: (monoid_add,dim_type,usys) monoid_add
  by (intro_classes; (transfer, simp add: eq_unit))
  
instance QuantT :: (comm_monoid_add,dim_type,usys) comm_monoid_add
  by (intro_classes; transfer, simp)

instantiation QuantT :: (uminus,dim_type,usys) uminus
begin
...
instantiation QuantT :: (minus,dim_type,usys) minus
begin
...
instance QuantT :: (linorder,dim_type,usys) linorder
  by (intro_classes, transfer, auto)

instantiation QuantT :: (scaleR,dim_type,usys) scaleR
begin
...
instance QuantT :: (real_vector,dim_type,usys) real_vector
  by (intro_classes, (transfer, simp add: eq_unit scaleR_add_left scaleR_add_right)+)

instantiation QuantT :: (norm,dim_type,usys) norm
begin
...
instantiation QuantT :: (sgn_div_norm,dim_type,usys) sgn_div_norm
begin
...
instantiation QuantT :: (dist_norm,dim_type,usys) dist_norm
begin
...
instantiation QuantT :: ("{uniformity_dist,dist_norm}",dim_type,usys) uniformity_dist
begin
...
instantiation QuantT :: ("{dist_norm,open_uniformity,uniformity_dist}",dim_type,usys) open_uniformity
begin
...
instance QuantT :: (real_normed_vector,dim_type,usys) real_normed_vector
